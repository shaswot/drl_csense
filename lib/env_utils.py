...
from stable_baselines3.common.atari_wrappers import StickyActionEnv, NoopResetEnv, MaxAndSkipEnv, EpisodicLifeEnv, FireResetEnv, WarpFrame, ClipRewardEnv, AtariWrapper
...
class NoisyWarp(gym.ObservationWrapper[np.ndarray, int, np.ndarray]):
    ...
    def observation(self, frame: np.ndarray) -> np.ndarray:
        ...
        if self.noise == 0.0:
            ...
            noisemask = self.np_random.choice(a=[False, True], size=frame.shape, p=[self.noise, 1.0-self.noise])
            frame = noisemask * frame
        frame = cv2.resize(frame, (self.width, self.height), interpolation=cv2.INTER_AREA)
        return frame[:, :, None]

class AtariWrapper_NoisyWarp(gym.Wrapper[np.ndarray, int, np.ndarray, int]):
    """
    Atari 2600 preprocessings
    ...
    * Clip reward to {-1, 0, 1}
    * Sticky actions: disabled by default
    * Noisy image: some random pixels are blacked out
    ...
    def __init__(
        ...
        if "FIRE" in env.unwrapped.get_action_meanings():  # type: ignore[attr-defined]
            env = FireResetEnv(env)
        env = NoisyWarp(env, width=screen_size, height=screen_size, noise=noise)
        if clip_reward:
            env = ClipRewardEnv(env)
...
class AtariWrapper_NoisyFrame(gym.Wrapper[np.ndarray, int, np.ndarray, int]):
    """
    Atari 2600 preprocessings
    ...
    * Noisy image: some random pixels are blacked out
    ...
    def __init__(
        ...
        if "FIRE" in env.unwrapped.get_action_meanings():  # type: ignore[attr-defined]
            env = FireResetEnv(env)
        env = WarpFrame(env, width=screen_size, height=screen_size)
        env = NoisyFrame(env, noise=noise)
        if clip_reward:
            env = ClipRewardEnv(env)
...
def make_atari_env_Custom(
    ...
    :param monitor_dir: Path to a folder where the monitor files will be saved.
        If None, no file will be written, however, the env will still be wrapped
        in a Monitor wrapper to provide additional information about training.
    :param wrapper_class: Custom Wrapper Class to implement # defaults to AtariWrapper
    :param wrapper_kwargs: Optional keyword argument to pass to the ``AtariWrapper_Compressed`` or ``AtariWrapper_NoisyFrame``
    :param env_kwargs: Optional keyword argument to pass to the env constructor
    :param vec_env_cls: A custom ``VecEnv`` class constructor. Default: None.
    :param vec_env_kwargs: Keyword arguments to pass to the ``VecEnv`` class constructor.
    :param monitor_kwargs: Keyword arguments to pass to the ``Monitor`` class constructor.
...
def make_atari_env_Custom_VecFrameStack(env_id: Union[str, Callable[..., gym.Env]], 
                                        n_envs: int = 1,
                                        monitor_dir: Optional[str] = None,
                                        wrapper_class: Optional[Callable[[gym.Env], gym.Env]] = AtariWrapper,
                                        seed: int = 0,
                                        wrapper_kwargs: Optional[Dict[str, Any]] = None):
    env = make_atari_env_Custom(env_id=env_id,
                                n_envs=n_envs,
                                monitor_dir = monitor_dir,
                                wrapper_class=wrapper_class,
                                seed=seed,
...
class MyAtariWrapper_nonflipped(gym.Wrapper[np.ndarray, int, np.ndarray, int]):
    """
    Atari 2600 preprocessings
    ...
    * Sparse image: some random pixels are blacked out
    ...
    def __init__(
        ...
        env = WarpFrame(env, width=screen_size, height=screen_size)
        if sparsity > 0.0:
            env = SparseView(env, sparsity=sparsity) # flips the image upside down by mistake :P
            env = UpsideDown(env) # reflip the image
        if clip_reward:
            env = ClipRewardEnv(env)
...
def make_my_atari_env_nonflipped(
    env_id: Union[str, Callable[..., gym.Env]],
    n_envs: int = 1,
    seed: Optional[int] = None,
    start_index: int = 0,
    ...
    return make_vec_env(
        ...
        start_index=start_index,
        monitor_dir=monitor_dir,
        wrapper_class=MyAtariWrapper_nonflipped,
        env_kwargs=env_kwargs,
        vec_env_cls=vec_env_cls,
        vec_env_kwargs=vec_env_kwargs,
        monitor_kwargs=monitor_kwargs,
...
def make_trial_env_nonflipped(env_id: Union[str, Callable[..., gym.Env]], 
                               n_envs: int = 1,
                               seed: int = 0,
                               sparsity: float = 0.0):

    trial_env = make_my_atari_env_nonflipped(env_id=env_id,
                                n_envs=n_envs,
                                seed=seed,
                                wrapper_kwargs={'sparsity': sparsity})
    trial_env = VecFrameStack(trial_env, n_stack=4)
